...
docker run -d -it -h cm01 --name cloudera-cm --rm -p 7180:7180 centos:6
...
# SSH/Hosts Conf
docker exec cloudera-cm bash -c "/etc/init.d/ssh start"
docker exec cloudera-db bash -c "/etc/init.d/ssh start"
